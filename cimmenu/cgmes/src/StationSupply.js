...
class StationSupply extends EnergyConsumer {
    ...
    static attributeHTML(object, cimmenu, classType="StationSupply") {
    ...
    static read(object) {
        Object.keys(object).forEach((attribute) => {
            if (!StationSupply.isMemberAttribute(attribute)) {
                console.error("Unexpected attribute for class StationSupply: ", attribute, " with value: ", object[attribute])
            }
        });
    }
    static renderAsClass(object, cimmenu) {
        let separateEntries = StationSupply.attributeHTML(object, cimmenu);
