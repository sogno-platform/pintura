...
const SVCControlModeEnum = {
        reactivePower:0,
        voltage:1,
}
const possibleValues = [
        { "value": "--"},
        { "value": "SVCControlMode.reactivePower", "label": "reactivePower" },
        { "value": "SVCControlMode.voltage", "label": "voltage" },
]

class SVCControlMode extends BaseClass {

    static attributeHTML(object, cimmenu, classType="SVCControlMode") {
    ...
    static read(object) {
        Object.keys(object).forEach((attribute) => {
            if (!SVCControlMode.isMemberAttribute(attribute)) {
                console.error("Unexpected attribute for class SVCControlMode: ", attribute, " with value: ", object[attribute])
            }
        });
    }
    static renderAsClass(object, cimmenu) {
        let separateEntries = SVCControlMode.attributeHTML(object, cimmenu);
