...
class DiagramStyle extends IdentifiedObject {

    static attributeHTML(object, cimmenu, classType="DiagramStyle") {
    ...
    static read(object) {
        Object.keys(object).forEach((attribute) => {
            if (!DiagramStyle.isMemberAttribute(attribute)) {
                console.error("Unexpected attribute for class DiagramStyle: ", attribute, " with value: ", object[attribute])
            }
        });
    }
    static renderAsClass(object, cimmenu) {
        let separateEntries = DiagramStyle.attributeHTML(object, cimmenu);
